...
class Trainer():
    ...
    def train(self, model, cfg_optim ,train_dataloader, eval_dataloader, val_dict):
        device = self.device
        model.to(device)
        optim = cfg_optim(model.parameters(), lr=self.lr)
        global_idx = 0
...
def objective(trial):
    ...
    best_scores = trainer.train(model, cfg['optimizer'], train_loader, val_loader, val_dict)
